	...
	bool isIdType)
{
	...
	this->AddPointInformation(data, ptr, numTuples,
		numComps, currentGlobalPointIndex, isIdType);
}
	...
	bool isIdType)
{
	...
	if (sizeof(T) == 8 && isIdType)//64位特殊处理
	{
		//offset all the pointers to the correct place
		T *src = buffer + ((start - currentGlobalIndex) * numComps);
		int *dest = (int*)pointData + (this->CurrentPointPropInfo->index * numComps);
		...
		for (; start < end; ++start, src += numComps, dest += numComps)
		{
