	...
	const bool& isRoadPoints)
{
	...
	{
		this->FillPointProperty<double>(numTuples, numComps, validParts, idx, isIdType);
	}
	else
	{
		this->FillPointProperty<float>(numTuples, numComps, validParts, idx, isIdType);
	}
	...
	bool isIdType)
{
	...
	for (vtkIdType j = 0; j < loopTimes; ++j, offset += numPointsToRead)
	{
		...
		while (partIt != sortedParts.end())
		{
			...
			(*partIt)->ReadPointBasedProperty(buf, numPointsToRead, numComps, offset, isIdType);
			++partIt;
		}
	}
	if (leftOver > 0 && !sortedParts.empty())
	{
		...
		for (partIt = sortedParts.begin(); partIt != sortedParts.end(); ++partIt)
		{
			(*partIt)->ReadPointBasedProperty(buf, leftOver, numComps, offset, isIdType);
		}
	}
